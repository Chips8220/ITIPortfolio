...
namespace Chapter10
{
    public partial class frmKWSales : Form
    {
        ...
        private void frmKWSales_Load(object sender, EventArgs e)
        {
            ...
            KWSalesConnection = new SqlConnection("Data Source=.\\SQLEXPRESS; AttachDbFilename=C:\\Database\\SQLKWSALES.mdf; Integrated Security=True; Connect Timeout=30; User Instance=True;");
            ...
            txtCity.DataBindings.Add("Text", customersTable,
            "City");
            txtState.DataBindings.Add("Text", customersTable,
            "State");
            txtZip.DataBindings.Add("Text", customersTable,
            "Zip");
            // establish currency manager
            customersManager = (CurrencyManager)
            this.BindingContext[customersTable];
        ...
        private void frmKWSales_FormClosing(Object sender, FormClosingEventArgs e)
        {
            if (newCustomer)
            {
                MessageBox.Show("You must finish the current edit before stopping.", "", MessageBoxButtons.OK,
            ...
            else
            {
                try
                {
                    ...
                    SqlCommandBuilder ordersAdapterCommands =
                    new
                    SqlCommandBuilder(ordersAdapter);
                    ordersAdapter.Update(ordersTable);
                    SqlCommandBuilder customersAdapterCommands
                    = new
                ...
                catch (Exception ex)
                {
                    ...
                    SqlCommandBuilder purchasesAdapterCommands = new SqlCommandBuilder(purchasesAdapter);
        ...
        private void FillCustomers()
        {
            ...
            {
                for (int nRec = 0; nRec <
                customersTable.Rows.Count; nRec++)
                {
                    cboCustomers.Items.Add(CustomerListing(customersTable
                    .Rows[nRec]["LastName"].ToString(),
        ...
        private void cboCustomers_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                customersTable.DefaultView.Find(ID);
            }
            catch (Exception ex)
            {
                MessageBox.Show("Could not find customer",
                "Search Error", MessageBoxButtons.OK, MessageBoxIcon.Information);
            }
        }
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            if (!allOK)
            {
                MessageBox.Show("All text boxes require an entry.",
                "Information Missing", MessageBoxButtons.OK,
                MessageBoxIcon.Information);
                txtFirstName.Focus();
                return;
            ...
            SqlConnection("Data Source=.\\SQLEXPRESS; AttachDbFilename=C:\\Database\\SQLKWSalesDB.mdf; Integrated Security=True; Connect Timeout=30; User Instance=True;");
            customersCommand = new SqlCommand("SELECT * FROM Customers", KWSalesConnection);
            customersAdapter = new SqlDataAdapter();
            customersAdapter.SelectCommand = customersCommand;
            customersTable = new DataTable();
            ...
            /// Find added customer
            string ID = "";
            for (int i = 0; i < customersTable.Rows.Count;
            i++)
            {
                if
                (customersTable.Rows[i]
                ["FirstName"].ToString().Equals(savedFirstName) &&
                customersTable.Rows[i]
                ["LastName"].ToString().Equals(savedLastName))
                {
                    ID =
        ...
        private void btnAdd_Click(object sender, EventArgs e)
        {
            ...
            for (int nRec = 0; nRec <
            productsTable.Rows.Count; nRec++)
            {
                if
                (productsTable.Rows[nRec]
                ["Description"].ToString().Equals(cboProducts.Text.ToString()))
                {
                    unitPrice = Convert.ToSingle(productsTable.Rows[nRec]["Price"]);
                    break;
                }
            }
            lstCart.Items.Add(nudQuantity.Value.ToString() + " " +
...
e)
        {
            ...
            if (lstCart.SelectedIndex != -1)
            {
                ...
                q = Convert.ToInt32(lstCart.Text.Substring(0,
                i));
                i = lstCart.Text.IndexOf("$");
                p = Convert.ToSingle(lstCart.Text.Substring(i +
                1, lstCart.Text.Length - i - 1));
                lblTotal.Text = String.Format("{0:f2}",
                Convert.ToSingle(lblTotal.Text) - q * p);
        ...
        private void btnSubmitOrder_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("You need to select a customer.", "Submit Error", MessageBoxButtons.OK,
                MessageBoxIcon.Information);
                return;
            }
            ...
            for (int i = 0; i < lstCart.Items.Count; i++)
            {
                ...
                int pr;
                for (pr = 0; pr < productsTable.Rows.Count;
                pr++)
                {
                    if
                    (productsTable.Rows[pr]
                    ["ProductID"].ToString().Equals(ID))
                    {
                        break;
                    }
                }
                productsTable.Rows[pr]["NumberSold"] =
                Convert.ToInt32(productsTable.Rows[pr]["NumberSold"])
                + q;
            }
        ...
        private void PrintInvoicePage(object sender,
PrintPageEventArgs e)
        {
            ...
            lblDate.Text, myFont, Brushes.Black, 100, y);
            y += 2 *
            Convert.ToInt32(myFont.GetHeight(e.Graphics));
            // Print buyer address
            ...
            y +=
            Convert.ToInt32(myFont.GetHeight(e.Graphics));
            e.Graphics.DrawString(txtAddress.Text, myFont,
            Brushes.Black, 100, y);
            y +=
            Convert.ToInt32(myFont.GetHeight(e.Graphics));
            e.Graphics.DrawString(txtCity.Text + ", " +
            txtState.Text + " " + txtZip.Text, myFont,
            Brushes.Black,
            100, y);
            y += 2 *
            Convert.ToInt32(myFont.GetHeight(e.Graphics));
            ...
            for (int i = 0; i < lstCart.Items.Count; i++)
            {
                ...
                j = desc.IndexOf("$");
                unit = desc.Substring(j + 1, desc.Length - (j +
                1));
                desc = desc.Substring(0, j - 1);
                if (desc.Length > 25)
                    desc = desc.Substring(0, 25);
                s = BlankLine(56); s = MidLine(q, s, 3 -
                q.Length);
                s = MidLine(id, s, 7);
                s = MidLine(desc, s, 15);
                s = MidLine(unit, s, 47 - unit.Length);
                t = String.Format("{0:f2}", Convert.ToSingle(q)
                * Convert.ToSingle(unit));
            ...
            s = MidLine("$" + lblTotal.Text, s, 55 -
            lblTotal.Text.Length);
            e.Graphics.DrawString(s, myFont, Brushes.Black,
            100, y);
        ...
        public string MidLine(string string1, string string2,
int p)
        {
            string s = "";
