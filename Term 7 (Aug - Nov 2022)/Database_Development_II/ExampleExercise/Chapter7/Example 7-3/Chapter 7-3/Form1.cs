...
namespace Chapter_7_3
{
    ...
    public partial class frmPubs : Form
    {
        ...
        private void frmPubs_Load(object sender, EventArgs e)
        {
            ...
            int l = 0;
            int w = Convert.ToInt32(this.ClientRectangle.Width
            / 3);
            int t = 0;
            int h =
            Convert.ToInt32(this.ClientRectangle.Height / 9);
            ...
            for (int i = 0; i < 27; i++)
            {
                ...
                btnLetter[i].Click += new
                EventHandler(this.btnLetter_Click);
                t += h;
                if (i == 8 || i == 17)
                {
                    l += w;
        ...
        private void btnLetter_Click(object sender, EventArgs
e)
        {
            ...
            // Add code handler
            publishersDocument.PrintPage += new
            PrintPageEventHandler(this.PrintPublishersPage);
            // Print document
            pageNumber = 1;
        ...
        private void PrintPublishersPage(object sender,PrintPageEventArgs e)
        {
            ...
            for (int i = recordsPerPage * (pageNumber - 1); i < iEnd; i++)
            {
                ...
                y += Convert.ToInt32(myFont.GetHeight());
                e.Graphics.DrawString("Address: " + publishersTable.Rows[i]["Address"].ToString(), myFont, Brushes.Black, e.MarginBounds.Left, y);
                y += Convert.ToInt32(myFont.GetHeight());
                e.Graphics.DrawString("City: " + publishersTable.Rows[i]["City"].ToString(), myFont,
                Brushes.Black, e.MarginBounds.Left, y);
                y += Convert.ToInt32(myFont.GetHeight());
                e.Graphics.DrawString("State: " + publishersTable.Rows[i]["State"].ToString(), myFont,
                Brushes.Black, e.MarginBounds.Left, y);
                y += Convert.ToInt32(myFont.GetHeight());
                e.Graphics.DrawString("Zip: " + publishersTable.Rows[i]["Zip"].ToString(), myFont, Brushes.Black, e.MarginBounds.Left, y);
                y += Convert.ToInt32(myFont.GetHeight());
